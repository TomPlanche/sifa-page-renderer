...
- a11e19d: Bring the "Now" page heatmap to parity with sifa-web's `/activity` Activity Bar: month + weekday axis labels, per-app dominant colours (the `--app-<id>-stripe` palette at the shared opacity ramp), a per-day hover breakdown showing each app's count and the total, and an app-colour legend below the grid. All derived from the existing `/api/activity/:id/heatmap` payload, so no API or wiring change is needed.
...
- 57ce146: Make the whole activity card link to its source record instead of a separate "View on {app}" text link. A stretched overlay anchor covers the card (opens the record on its origin app in a new tab); the card's inner links (embedded reply/repost subject, external links) stay independently clickable above the overlay.

## 0.2.21

### Patch Changes

- dc9a157: Activity stream: link each card to its record on the origin app (a "View on {source}" affordance in the meta row, opening in a new tab) and render replies with a "Replying to" caption above the embedded original post. Bumps the `@singi-labs/sifa-sdk` peer to 0.12.15 for `StreamCardVM.sourceUrl`.

## 0.2.20

### Patch Changes

- a74366f: Two personal-site tweaks: drop the shipped brand logo (it was an inaccurate hand extraction) so every activity card icon uses the category glyph, and remove the duplicate `@handle` line under the sidebar name (the handle already appears as the Bluesky link in the links section).
